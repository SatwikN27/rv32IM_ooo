...
- Either Tomasulo's Algorithm or an Explicit Register Renaming (ERR) scheme
...
well as some staff-provided baselines.  While this will not impact your grade, you get bragging rights for doing better than your peers!
...
exception of staff-provided resources. In addition, you may use any DesignWare IPs
...
website](https://courses.grainger.illinois.edu/ece411/sp2026/assignments.html#late-policy). Unlike
...
- Fill out the form for group and architecture selection on Campuswire (1 per group) [1]
...
have your processor connected to the RVFI/Spike monitor via `configs/rvfi_reference.yaml`.
...
- $PD^4$, where $P$ is power and $D$ is delay
  - Power is reported by AG's `scons power PROG=prog.elf` in mW
...
300K as reported by Synopsys DC. For violations over this limit, we penalize performance by multiplying $PD^4$ and dividing IPC by $2^{\frac{A - 300,000}{50,000}}$, where $A$ is the area
as reported by DC.
...
conservative implementation of a CP3 core. On each of the 6 CP3 benchmarks, your design will be compared against the baseline using the following metrics:

- IPC (must be higher than baseline)
  - Coremark, FFT, AES-SHA, Image: 0.5 each
  - Compression, Mergesort: 0.25 each

- $PD^4$ (must be lower than baseline)
  - Coremark, FFT, AES-SHA, Image: 1.5 each
  - Compression, Mergesort: 0.75 each
...
in the table below. All metrics are rounded to three decimal places.

| Benchmark   | IPC   | Power (mW) | Delay (ms) | $PD^4$    |
|-------------|-------|------------|------------|-----------|
| coremark_im | 0.398 |   18.858   |   1.466    | 87.187    |
| aes_sha     | 0.309 |   18.598   |   5.022    | 11826.027 |
| compression | 0.484 |   18.917   |   1.776    | 188.395   |
| fft         | 0.508 |   18.872   |   4.570    | 8229.957  |
| mergesort   | 0.431 |   18.865   |   2.236    | 471.743   |
| image       | 0.376 |   18.821   |   2.513    | 750.796   |
...
- Be synthesizable and meet timing at the frequency you specify in `configs/config.yaml`
...
Presentations will happen on **05/06**. Your final report will be due on **05/08** at midnight.
Please push your final report to Github before the deadline. 
...
scons audit                                         # RTL banned words and patterns check, not spyglass linting
scons sim PROG=prog.elf                             # Run default testbench (top_tb)
scons sim PROG=prog.elf SIM_TB=foo DUMP_FSDB=0      # Run specific testbench, don't dump FSDB
scons syn                                           # Synthesis only
scons power PROG=prog.elf                           # Full flow (sim -> saif -> power)

# Debug
scons --debug=explain syn  # Show why rebuilding
scons --tree=all syn       # Show dependency tree
```
