module tb_wrapper;
    ...
    time dram_tick_period_ps, dram_tick_half_period_ps;
    ...
    initial begin
        ...
        if (!$value$plusargs("DRAM_TICK_PERIOD_PS=%d", dram_tick_period_ps)) begin
            $fatal(1, "ERROR: DRAM_TICK_PERIOD_PS plusarg not defined.");
        end

        if (dram_tick_period_ps < 2) begin
            $fatal(1, "ERROR: DRAM_TICK_PERIOD_PS must be >= 2 ps (got %0d).", dram_tick_period_ps);
        end

        dram_tick_half_period_ps = dram_tick_period_ps / 2;
        if (dram_tick_half_period_ps == 0) begin
            $fatal(1, "ERROR: Computed DRAM tick half period is 0 ps.");
        end
    ...
    initial begin
        wait (dram_tick_half_period_ps > 0);
        forever #(dram_tick_half_period_ps) dram_clk = ~dram_clk;
    end
