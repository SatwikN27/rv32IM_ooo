...
based on your processor design.  Inside `configs/config.yaml` there are a few parameters 
that you will need to set:

- `clock_period` - Clock period of your processor in picoseconds (ps). Make sure your
  processor can synthesize at this frequency, otherwise the autograder won't run
  your code!
- `dw_ips` - The list of Synopsys IP that you wish to use. 
- `arch` - Default is rv32im. If you wish to support compressed instructions, change to rv32imc. 

# DRAM Parameters
You may also change the parameters of the DRAM model using `configs/dram.yaml`. The parameters
you can change are:
...
Then, put them in `configs/config.yaml` with the proper format.
