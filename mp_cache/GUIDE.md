...
## Loading files into `memory_model.sv`

The provided memory model loads a memory file using `$readmemh` from
`tests/program.hex`. This file should be in
...
Section 21.4. You should write your own memory file with contents of your choice, though
an example is provided. Alternately, you can modify `memory_model.sv` to have 
random data using SystemVerilog's randomization features.

Let's say your memory file is located at `tests/program.hex`.
...
In most cases, the above command would suffice for testing, as loading data to the dram model is already handled.
...
To use OpenRAM, first prepare a config file(s) in `sram/`.
We have provided the SRAM config files you need to use for this MP.
Scripts in the provided file will convert them into the actual
configuration file used by OpenRAM.

Once the config file is prepared, run:

```bash
$ scons sram
```

This will generate all relevant files, but the generated artifacts
will not be exposed to your local workspace to improve performance.
When running simulation, a cached version of the SRAM artifacts is
loaded into your simulation environment.
The build system also converts the timing model to a format that DC 
can use. This timing model is used by the provided synthesis script.
