...
3. **Constrained random and coverage:** Writing directed test vectors isn't a lot
   ...
   collection to better verify complex designs. It may be useful to use your
   solution to this part when completing part 4.
4. **Verification of a CPU:** Finally, a CPU! You will find
   bugs in a simple multicycle RISC-V CPU implementation.
...
First, see the `mp_verif` folder structure with:
...
ECE 411:
- `rtl` contains synthesizable RTL.
- `tb` contains non-synthesizable testbenches used for simulation and
  verification.
- `lint` contains scripts to run lint on your design in `rtl`
- `sim` contains scripts to run simulation on your design in `rtl`
- `syn` contains scripts to run synthesis your design in `rtl`

For now, your LFSR design will go in `rtl/lfsr/lfsr.sv`. For this part of the
MP, the testbench is provided fully implemented in `tb/lfsr_tb/`. In
...
`rtl/lfsr/lfsr.sv`, run the testbench in simulation with:

```bash
$ scons sim PROFILE=lfsr_tb
```
...
```bash
$ scons lint PROFILE=lfsr_tb
```

You can check your report in `build/lint/lfsr_tb/lint.rpt`.
...
```bash
$ scons syn PROFILE=lfsr_tb
```

Once synthesis finishes running (and you get the "Synthesis
Successful" text), check out the reports in `build/syn/lfsr_tb/`. The two
...
## Issues with `rtl/alu/alu.sv`

The data above is enough to write a very quick implementation of the
ALU, as provided in `rtl/alu/alu.sv`. However, this design has bugs, as
...
```bash
$ scons syn PROFILE=alu_tb
```

Reading `build/syn/alu_tb/synthesis.log` will tell you which variables in the
ALU became inferred latches.

```bash
$ scons lint PROFILE=alu_tb
```

Reading `build/syn/alu_tb/lint.rpt` will tell you which variables in the
ALU became inferred latches.
...
`tb/alu_tb/alu_tb.sv`. You should first read through this file to make sure
...
written for you, and is in `tb/alu_tb/coverage.sv`. For you to get full
...
```bash
$ scons sim PROFILE=alu_tb
$ scons covrep PROFILE=alu_tb
```
...
- If you're on FastX/X-forwarding or in person at the lab:
  ```bash
  $ firefox ./build/sim/vcs/alu_tb/urgReport/dashboard.html
  ```
- If you're on SSH and prefer to use your local web browser, on your
  ...
  ```
  Then, on EWS, navigate to `mp_verif/build/sim/vcs/alu_tb/urgReport`,
  and do:
  ```bash
...
In the navbar, click on "Groups", then on something similar to "tb_wrapper::cg" in the second
...
coverage will go up. Rerun the sim and coverage report commands, then
...
Simply run lint and check `build/lint/loop_tb/lint.rpt`.
...
`build/synth/loop_tb/synthesis.log`. You might have noticed that the loop is
...
```bash
$ scons syn PROFILE=loop_tb
$ scons dv PROFILE=loop_tb
```

By default, the right hand side of the main window will list all the cells in your design
organized by hierarchy.

- Find the line in `build/syn/loop_tb/synthesis.log` that shows you the loop
- Switch the menu option from cells(heirarchical) to cells(all)  
...
a timing report will be generated at `build/syn/tree_tb/timing.rpt`.
...
cardinality of this cross product is 9216: that is, there are 9216
...
```bash
$ scons sim PROFILE=cpu_tb PROG=./tests/riscv_basic_asm.s
```
...
Our provided scons script converts one of:
...
- `sim/vcs/cpu_tb/bin/*.elf`, compiled from your provided source if not already in ELF format
- `sim/vcs/cpu_tb/bin/*.dis`, the disassembly of your provided source
- `sim/vcs/cpu_tb/bin/*.hex`, a memory file format (hex) supported by SystemVerilog `$readmemh` task
...
```bash
$ scons spike PROFILE=cpu_tb PROG=./tests/riscv_basic_asm.s
```
...
is `0x55556000` in length starting from `0xaaaaa000`. Keep this in mind when
