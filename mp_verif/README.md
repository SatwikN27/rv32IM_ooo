...
Files related to this part are in `mp_verif/rtl/lfsr` and `mp_verif/tb/lfsr_tb`.
...
Files related to this part are in `mp_verif/rtl/alu` and `mp_verif/tb/alu_tb`.

You are to fix a given ALU, located at `rtl/alu/alu.sv`.
...
- `tb/alu_tb/*`: The autograder will exercise your testbench on
  a reference design to check that your testbench has 100% coverage.
- `rtl/alu/alu.sv`: The autograder will run a reference testbench on
  your ALU implementation. Your ALU must also pass synthesis and timing.


## Part 2.2: Combinational Loops

Files related to this part are in `mp_verif/rtl/loop`.
...
Files related to this part are in `mp_verif/rtl/tree` and `mp_verif/tb/tree_tb`.

The provided design in `rtl/tree` does not pass timing.
...
Files related to this part are in `mp_verif/rtl/rand` and `mp_verif/tb/rand_tb`.
...
The constraints must be written in the file `tb/rand_tb/randinst.sv`. The
covergroup is in the file `tb/rand_tb/instr_cg.sv`. The files have TODOs
...
Files you will be graded on:
- `rtl/rand/pkg/types.sv`
- `tb/rand_tb/randinst.sv`
- `tb/rand_tb/instr_cg.sv`


# Part 4: CPU Verification

Files related to this part are in `mp_verif/rtl/cpu` anf `mp_verif/rtl/cpu`.

You are provided a multicycle RISC-V RV32I CPU in `rtl/cpu`.
This design has numerous functional bugs that you are required to find
by writing assembly (directed tests) and via randomized testing from `rand_tb`.
...
Files you will be graded on:
- `rtl/cpu/cpu.sv`, `rtl/cpu/regfile.sv`: The CPU you submit must fix all provided bugs and correctly
  implement the RISC-V spec, with the exception of `FENCE*`, `ECALL`, `EBREAK`, and `CSRR*` instructions.
- `rtl/cpu/types.sv`: types definition for the CPU.
...
## Synthesis
We will run synthesis on any `rtl` code you submit for this course. Running synthesis
...
## Lint
We will also run lint on any `rtl` code you submit for this course.
